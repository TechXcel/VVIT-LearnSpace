...
const createProjectReview = asyncHandler(async (req, res) => {
  ...
  return res
    .status(201)
    .json(
      new ApiResponse(200, { review: newReview }, "Review added successfully")
    );
});
...
const getReviewByProjectId = asyncHandler(async (req, res) => {
  ...
  const review = await Review.findOne({ projectId: reviewId });
  console.log(review);
  if (!review) {
...
const deleteReviewByProjectId = asyncHandler(async (req, res) => {
  ...
  const review = await Review.findOneAndDelete({ projectId: reviewId });
...
const deleteReviewByResourceId = asyncHandler(async (req, res) => {
  ...
  const review = await Review.findOneAndDelete({ resourceId: reviewId });
